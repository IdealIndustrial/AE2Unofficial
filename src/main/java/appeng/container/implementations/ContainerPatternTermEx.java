...
public class ContainerPatternTermEx extends ContainerMEMonitorable implements IAEAppEngInventory, IOptionalSlotHost, IContainerCraftingPacket
{
	...
	public void encode()
	{
		...
		encodedValue.setBoolean( "newSaveStyle", true );

		for( int i = 0; i < in.length; i++ )
		{
			encodedValue.setLong( "incnt" + i, in[i] == null ? 0 : in[i].stackSize );
		}

		for( int i = 0; i < out.length; i++ )
		{
			encodedValue.setLong( "outcnt" + i, out[i] == null ? 0 : out[i].stackSize );
		}
